...
1. **Fixtures green.** Shared workflows must pass the executable fixtures. Those fixtures are on still-open [engineering-standards#8](https://github.com/Robben-Media/engineering-standards/pull/8). They are not on `main`. This gate is **not** cleared. No Actions run on `#8` is claimed here.
...
Pin `@v1`, not `@main`. Jeremy moves `v1`. Rollback is retag, documented when `#8` lands. Until then, `v1` is `472bfa1`.
